...
function p.payload(level, bp)
    p.payload_new(level, bp)
end

--Deprecated this in a later version.
-- function p.payload_old(level, bp)
--     local center = std.block_center(bp)
--     local worked = ga_tele_pink2(center) --Deprecated.
-- end

--The new way of doing things.
function p.payload_new(level, bp)
...
function p.__get_use_msg(level, bp)
    return "Grow until find pink sphere"
end
