...
function p.__process_input(wid)
    ...
    then
        ...
        win_base_wp.advance_wp_unhide_sortby_int()
        p.refresh_options(wid)
    end
...
function p.__render(wid)
    ...
    ga_win_txt_center(wid, 0.9, "SELECT A WAYPOINT TO UNHIDE")

    ga_win_set_char_size(wid, 0.01, 0.02)
    ga_win_txt_center(wid, 0.84, "SEARCH:")

    win_base_wp.render_sortby_unhide(wid)
end
...
function p.unhide_wp(wid, wp_handle)
