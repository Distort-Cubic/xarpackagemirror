...
function p.__process_input(wid)
    ...
    if( sel_num >= 0 ) then
        local sel_handle = cur_handles[sel_num + 1] --Lua arrays are 1 indexed.
        if( sel_handle == nil ) then
            ga_print("*** Error: In win_base_wp_travel.__process_input, the selected handle is nil")
            ga_exit()
        end
        p.teleport(sel_handle) --Actually doing something.
        ga_window_pop_all()
    end
end
...
function p.__render(wid)
    ...
    ga_win_txt_center(wid, 0.9, "SELECT A WAYPOINT TO TRAVEL TO")
...
--Actually teleporting (and then hiding).
function p.teleport(target_handle)
    local source_handle = win_base_wp.get_cur_handle()
    game_base_wp_system.teleport(source_handle, target_handle)
    --
    local source_path = game_base_wp_system.convert_handle_to_path(source_handle)
    game_base_wp_system.toggle_hidden(source_path)
end
