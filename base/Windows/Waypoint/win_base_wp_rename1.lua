...
--We wouldn't need to do this if we had "__open" and "__close" functions.
function p.make_dirty()
    dirty = true
end
...
function p.__process_input(wid)
    ...
    if( sel_num >= 0 ) then
        ...
        if( sel_handle == nil ) then
            ga_print("*** Error: In win_base_wp_rename1.__process_input, the selected handle is nil")
            return
        end
        win_base_wp_rename2.set_cur_handle(sel_handle)
        ga_window_push("win_base_wp_rename2")
    end
end
...
function p.__render(wid)
    ...
    ga_win_txt_center(wid, 0.9, "SELECT A WAYPOINT TO RENAME")
