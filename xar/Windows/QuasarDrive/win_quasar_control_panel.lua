...
function p.make_list(wid)
    local use_alt_radius = game_quasar_drive.get_uses_alt_radius()
    local value1 = (
        use_alt_radius
        and "TRUE"
        or "FALSE" )
    ...
    if( use_alt_radius ) then
        local alt_radius = game_quasar_drive.get_alt_radius()
...
function p.__render(wid)
    ...
    ga_win_txt_center(wid, 0.9, "QUASAR DRIVE")
...
function p.__process_input(wid)
    ...
    if( string.match(sel_name, "USE ALTERNATE LEVEL RADIUS") ) then
        game_quasar_drive.toggle_uses_alt_radius()
        p.make_list(wid)
    end
    if( string.match(sel_name, "SET ALTERNATE LEVEL RADIUS") ) then
        open = false
        ga_window_push("win_quasar_control_panel_helper")
        return
    end
end
