...
function p.__render(wid)
    ...
    ga_win_txt_center(wid, 0.85, "CAUSTIC EUTOPIA MODE")
    ...
    ga_win_txt_center(wid, 0.80, "DAMAGE TO ENEMIES: 50%")
    ga_win_txt_center(wid, 0.75, "DAMAGE FROM ENEMIES: 139%")
    -- ga_win_txt_center(wid, 0.75, "DAMAGE FROM ENEMIES: 125%") --Not accurate.

    ga_win_quad(wid, 0.35, 0.3, 0.65, 0.7, "icon_hard_3")
end

function p.__process_input(wid)
    if ga_win_key_pressed(wid, "Y") then
        --Actually changing the difficulty.
        game_difficulty.set(3)
        ga_play_sound("harp")
        ga_window_pop_all()
        return
    end
