...
function p.create_tab_buttons(wid)
    ...
    ga_win_widget_button_start(wid, 111, x, y, w, h, "MYLANTIS")
...
function p.__render(wid)
    ...
    if win_num == 5 then
        ...
        p.draw_trophy(wid, grid, 0,1, "i2_mountains")
    end
    if win_num == 6 then
    ...
    if win_num == 9 then
        ...
        p.draw_trophy(wid, grid, 2,2, "black_hole_big")
        p.draw_trophy(wid, grid, 2,3, "sponge_alpha")
    end
    if win_num == 10 then
    ...
    if win_num == 13 then
        ...
        p.draw_trophy(wid, grid, 0,3, "health_tower")
        p.draw_trophy(wid, grid, 1,0, "alpha_cube")
    end
end
...
function p.__process_input(wid)
    ...
    if click_sel == "MYLANTIS"   then num = 11 end
