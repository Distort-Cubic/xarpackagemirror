...
function p.__render(wid)
    ...
    if win_num == 2 then
        ...
        --Mode 4.
        if game_wep_modes.get_equipped(2,4) then
            cur_y = cur_y - step_y

            cur_y = cur_y - step_y
            local damage = p.calc_dmg_from_player(
                game_wep_modes.get_curved_damage(2,4))
            ga_win_txt(wid, min_x,cur_y, "DAMAGE (The Sheriff): " ..
                game_str.add_commas( math.floor(damage) ) )

            cur_y = cur_y - step_y
            ga_win_txt(wid, min_x,cur_y, "DPA (The Sheriff): " ..
                game_str.add_commas( math.floor(game_wep_modes.get_dpa(2,4)) ) )

            cur_y = cur_y - step_y
            local fire_rate = 1.0 / game_wep_modes.get_fire_period(2,4)
            ...
            local mcolor = p.max_color(
                "xar.player.gun2.speed_level",
                game_wep_modes.get_vel_level_max(2))
            local speed2 = game_wep_modes.get_vel(2,4)
            ga_win_txt(wid, min_x,cur_y, mcolor .. "VELOCITY (The Sheriff): " .. speed2 .. " per sec")
        end

        --Mode 5.
        if game_wep_modes.get_equipped(2,5) then
            cur_y = cur_y - step_y

            cur_y = cur_y - step_y
            local damage = p.calc_dmg_from_player(
                game_wep_modes.get_curved_damage(2,5))
            ga_win_txt(wid, min_x,cur_y, "DAMAGE (The Judge): " ..
                game_str.add_commas( math.floor(damage) ) )

            cur_y = cur_y - step_y
            ga_win_txt(wid, min_x,cur_y, "DPA (The Judge): " ..
                game_str.add_commas( math.floor(game_wep_modes.get_dpa(2,5)) ) )

            cur_y = cur_y - step_y
            local fire_rate = 1.0 / game_wep_modes.get_fire_period(2,5)
            ga_win_txt(wid, min_x,cur_y, "FIRE RATE: " .. p.round(fire_rate,2) .. " per sec")
            ...
            local mcolor = p.max_color(
                "xar.player.gun2.speed_level",
                game_wep_modes.get_vel_level_max(2))
            local speed2 = game_wep_modes.get_vel(2,5)
            ga_win_txt(wid, min_x,cur_y, mcolor .. "VELOCITY (The Judge): " .. speed2 .. " per sec")
        end
