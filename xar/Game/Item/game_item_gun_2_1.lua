...
function p.use_common(mode)
    ...
    for i = 1,num do
        local ent_type = ""
        if mode == 1 then ent_type = "ment_player_proj_cannon_1" end
        if mode == 2 then ent_type = "ment_player_proj_cannon_2" end
        if mode == 3 then ent_type = "ment_player_proj_cannon_3" end
        if mode == 4 then ent_type = "ment_player_proj_cannon_4" end
        if mode == 5 then ent_type = "ment_player_proj_cannon_5" end

        local viewer_pos = ga_get_viewer_offset()
        local look = ga_get_sys_v("game.player.camera.look")
        local extend_len = 1.0
        if( mode == 2 ) then extend_len = 2 end
...
function p.get_use_period()
    return game_wep_modes.get_fire_period(2,1)
end
