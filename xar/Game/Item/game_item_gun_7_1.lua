...
function p.use_common(mode)
    ...
    if mode == 2 then ent_type = "ment_player_proj_railgun_2" end
    if mode == 3 then ent_type = "ment_player_proj_railgun_3" end
    if mode == 4 then ent_type = "ment_player_proj_railgun_4" end
    ...
    --Kickback.
    local kf = 0.0 --Kickback factor.
    if mode == 1 then kf = 0.20 end
    if mode == 2 then kf = 0.0 end
    if mode == 3 then kf = -0.15 end --Kick forward.
    if mode == 4 then kf = -0.15 end --Kick forward.
...
function p.get_use_period()
    return game_wep_modes.get_fire_period(7,1)
end
